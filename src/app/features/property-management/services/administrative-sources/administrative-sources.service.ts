...
export class AdministrativeSourcesService {
  ...
  getAdministrativeSourcesMain(
    baunitId: string
  ): Observable<AdministrativeSource[]> {
    const url = `${this.base_url}main/${baunitId}`;
    return this.http.get<AdministrativeSource[]>(url).pipe(
      catchError((error) => {
        throw error;
      })
    );
  ...
  getAdministrativeSourceById(params: {
    id: number;
    baunitId: string;
    executionId: string;
  }): Observable<AdministrativeSource> {
  ...
  createAdministrativeSource(
    params: CreateAdministrativeSourceParams
  ): Observable<AdministrativeSource> {
    const { executionId, baunitId, administrativeSource } = params;
    const url = `${this.base_url}temp/${executionId}/${baunitId}`;

    return this.http.post<AdministrativeSource>(url, administrativeSource).pipe(
      catchError((error) => {
        throw error;
      })
    );
  ...
  /* PUT */
  updateAdministrativeSource(
    parameters: UpdateAdministrativeSource
  ): Observable<AdministrativeSource> {
    const { baunitId, executionId, params } = parameters;
    const url = `${this.base_url}temp/${executionId}/${baunitId}`;

    return this.http.put<AdministrativeSource>(url, params).pipe(
      catchError((error) => {
        throw error;
      })
    );
  ...
  deleteAdministrativeSource(
    params: DeleteAdministrativeSourceParams
  ): Observable<void> {
