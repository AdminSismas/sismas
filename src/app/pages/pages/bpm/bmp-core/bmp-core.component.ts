...
export class BmpCoreComponent implements OnInit {
  ...
  executionId$ = this.route.params.pipe(map((params) => params[CONSTANT_NAME_ID]));
  ...
  refreshComponentsDynamic(proFlow: ProFlow) {
    ...
    const components: BasicComponentTemplate[] = LISTO_FORM_BPM_CORE.filter(x => x.pathForm.includes(pathForm)
    );
    const listComponents: ComponentTemplate[] = [];
    if (components?.length > 0) {
      components.forEach((component: BasicComponentTemplate) => {
        const listTmp = this.listComponents.filter(
  ...
  nextBpmCore() {
    ...
    if(this.proFlow) {
      ...
      const components: BasicComponentTemplate[] =
        LISTO_FORM_BPM_CORE.filter(x => x.pathForm.includes(pathForm)
      );
  ...
  executeBpmNextBpmCore(){
    this.bpmCoreService.getNextOperation(this.executionId)
      .subscribe(
        {
  ...
  previewBpmCore() {
    this.activateLoading();
    this.bpmCoreService.getPreviewOperation(this.executionId)
      .subscribe(
        {
          error: () => this.captureInformationSubscribeError(),
          next: (result: ProTaskE) => this.captureInformationBpmCore(result)
  ...
  getNewProFlow(flowId: string) {
    this.bpmCoreService.getProFlow(flowId)
      .subscribe((result: ProFlow) => {
  ...
  captureInformationSubscribeError(): void {
    this.activateLoading(true);
    this.activateSnapError(
      'Error ejecutando servicio de continuar flujo Bpm o retroceder, no es posible continuar', 3000);
    return;
  }
