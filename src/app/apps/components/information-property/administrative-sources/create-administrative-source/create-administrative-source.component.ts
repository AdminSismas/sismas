...
export class CreateAdministrativeSourceComponent implements OnInit {
    ...
    this.administrativeSourceService.createAdministrativeSource(params)
      .subscribe({
        next: (data: AdministrativeSource) => {
          this.snackbar.open('Fuente administrativa creada', 'CLOSE', { duration: 10000 });
          this.dialogRef.close(data);
        },
        error: (error: any) => {
          this.snackbar.open('Error al crear la fuente administrativa', 'CLOSE', { duration: 10000 });
        }
      });
  }
  ...
  editAdministrativeSourceFunction(): void {
    ...
    this.administrativeSourceService.updateAdministrativeSource({
      ...
      .subscribe({
        next: (data: AdministrativeSource) => {
          this.snackbar.open('Fuente Administrativa actualizada', 'CLOSE', { duration: 10000 });
          this.dialogRef.close();
        },
        error: (error: any) => {
          this.snackbar.open('Error al actualizar la fuente administrativa', 'CLOSE', { duration: 10000 });
        }
      });
  }
}
